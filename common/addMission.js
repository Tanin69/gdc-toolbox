...
exports.addMission = function (jsRes) {
    ...
    console.log(`${DBG_PREF} ${receivedPboName} : début de l'ajout de la mission à la base de données`);
    //console.log(`${DBG_PREF} ${receivedPboName} : JSON reçu...`);
    //console.log(jsRes);
    ...
    Mission.findOneAndDelete({"missionPbo.val": receivedPboName}, function(err, data){    
        if(err){
            ...
            console.log(`${DBG_PREF} ${receivedPboName} : l'enregistrement précédent de la mission a été supprimé de la base`);
        }
    });
    ...
    mission.save(function (err, mission) {
        if(err){
            console.log(`${DBG_PREF} ${receivedPboName} : erreur durant la sauvegarde de la mission`);
            return next(err);
        } else {
